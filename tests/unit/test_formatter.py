...
def test_stats_empty_series_returns_all_none():
    ...
    assert stats == {"mean": None, "min": None, "max": None}
...
def test_stats_computes_mean_min_max():
...
def test_value_line_renders_mean_and_status():
    stats = {"mean": 20.0, "min": 8.0, "max": 25.0}
...
def test_minmax_line_renders_both_pollutants():
    stats_by_label = {
        "PM2.5": {"mean": 12.2, "min": 8.4, "max": 21.0},
        "PM10": {"mean": 27.9, "min": 19.2, "max": 40.3},
    }
...
def test_minmax_line_placeholder_for_missing_values():
    stats_by_label = {
        "PM2.5": {"mean": None, "min": None, "max": None},
        "PM10": {"mean": None, "min": None, "max": None},
    }
...
def test_alert_when_mean_exceeds_threshold():
    stats = {"mean": 30.0, "min": 25.0, "max": 40.0}
    alerts = _pollutant_alerts("PM2.5", stats, threshold=15.0)
    assert alerts == ["   PM2.5 is 2.0x the safe limit"]


def test_no_alert_when_mean_below_threshold():
    stats = {"mean": 12.0, "min": 5.0, "max": 24.0}
...
def test_no_alert_when_below_everything():
    stats = {"mean": 5.0, "min": 3.0, "max": 7.0}
    assert _pollutant_alerts("PM2.5", stats, threshold=15.0) == []
...
def test_format_message_moderate_data_reports_all_clear(fake_air_data):
    message = format_message("milano", fake_air_data)
    assert "ALL CLEAR" in message
    assert "hotspot" not in message
